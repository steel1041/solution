...
namespace SDUSDTContract1
{
    public class SDUSDTContract : SmartContract
    {
        ...
        private static bool Redeem(byte[] addr)
        {
            ...
            //保存剩余PNEO金额
            var txid = ((Transaction)ExecutionEngine.ScriptContainer).Hash;
            Storage.Put(Storage.CurrentContext,txid,currentRemain);
            //拿到该有的PNEO
        ...
        private static Boolean Bite(byte[] otherAddr, byte[] addr)
        {
            ...
            //保存剩余PNEO金额
            var txid = ((Transaction)ExecutionEngine.ScriptContainer).Hash;
            Storage.Put(Storage.CurrentContext, txid, canClearPneo);
        ...
        private static Boolean Shut(byte[] addr)
        {
            ...
            if (locked > 0)
            {
                //Storage.Put(Storage.CurrentContext, txid, locked);
        ...
        private static Boolean Free(byte[] addr, BigInteger freeMount)
        {
            ...
            var txid = ((Transaction)ExecutionEngine.ScriptContainer).Hash;
            //增发PNEO
            Storage.Put(Storage.CurrentContext, txid, freeMount);
